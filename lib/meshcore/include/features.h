...
// Флуд-сеть живёт тем, что каждый услышавший переиздаёт кадр: без этого сообщение не
// уходит дальше одного хопа. Переиздание ест ток эфира и приёмника, поэтому на батарейном
// узле его можно отключить (-DFEATURE_RELAY=0) — узел останется в сети, но сам не форвардит.
#ifndef FEATURE_RELAY
