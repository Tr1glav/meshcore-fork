...
#define SENSOR_MSG_HELLO_REQ "hello?"
// Узел-прошивальщик объявляет себя и свой адрес в сети: "support:<ip>". Отдельным
// сообщением, а не полем в hello, — его шлёт один узел из всех, и платить за это лишними
// байтами в каждом heartbeat каждого узла незачем. Адрес нужен потому, что образ уходит
// к нему по сети, а не по радио: мегабайт по LoRa не передашь.
#define SENSOR_MSG_SUPPORT "support:"   // бот просит сенсоры отметиться (кнопка на странице OTA)
...
// Перед тем как погаснуть совсем, экран приглушается: на устройство обычно поглядывают
// мельком, и резкий переход «полная яркость — темнота» раздражает сильнее, чем тусклый,
// но читаемый экран. У цветной панели это ещё и заметная доля тока подсветки.
#define SCREEN_DIM_MS (30UL * 1000)
#define SCREEN_DIM_PERCENT 20
...
// быть случайным. Раньше макрос лежал под #ifdef MQTT_ENABLED и узлам был не виден.
...
// Потолок хопов, до которого кадр переиздаётся. При PATH_HASH_SIZE=2 путь в 64 байта —
// это ровно 32 хопа, о чём и говорит комментарий у PATH_LEN_INIT: дальше сеть слишком
// велика для флуда, и кадр просто не форвардим.
#define MAX_RELAY_HOPS 32
// Сколько чужих флудов одновременно ждут переиздания. Отправитель повторяет свой кадр,
// а другие услышавшие ретрансляторы должны разойтись по времени, поэтому передача
// откладывается, а не идёт сразу, — без маленькой очереди кадры, пришедшие в паузе,
// терялись бы.
#define RELAY_QUEUE_MAX 8
// Случайная пауза перед переизданием. Покрывает повторы отправителя (два кадра с
// паузой 60 мс и эфиром ~0.5 с) и разводит по времени ретрансляторов, услышавших
// один и тот же кадр, — при одинаковой паузе они глушили бы друг друга.
