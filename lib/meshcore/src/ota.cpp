...
// Сессия по радио: проверка пути до узла и собственно старт. Вынесено отдельно, потому
// что сюда приходят двумя дорогами — сразу, когда прошивальщика нет, и позже, когда
// передача ему не удалась и мы пробуем дотянуться сами из главного цикла.
static bool otaStartRadio(const String& target) {
...
bool otaStartSession(const String& target) {
    ...
    // supportFlashSelf).
    // Обе передачи прошивальщику идут фоновой задачей: в них мегабайт по сети, десятки
...
// Итог фоновой передачи прошивальщику. Разбирается в главном цикле, а не в самой задаче:
// otaDelegate и otaNote читает обработчик страницы, и писать их должен один поток.
void otaSupportTick() {
